      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-sm text-gray-500">
            {passport.sessions_completed} sessions
          </span>
        </div>
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <div className="font-semibold">
              {(passport.metrics.debug_efficiency * 100).toFixed(0)}%
            </div>
          </div>
          <div>
            <span className="text-gray-500">Integrity</span>
            <div className="font-semibold">
              {(passport.metrics.integrity * 100).toFixed(0)}%
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="bg-white dark:bg-slate-800 rounded-lg p-6 space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="text-right">
          <div className="text-2xl font-bold text-primary-600">
            {passport.tasks_passed}/{passport.sessions_completed}
          </div>
      ...
      <div className="pt-4 border-t border-gray-200 dark:border-slate-600 text-sm text-gray-500">
        <div className="flex justify-between">
          <span>Archetype confidence: {(passport.archetype.confidence * 100).toFixed(0)}%</span>
